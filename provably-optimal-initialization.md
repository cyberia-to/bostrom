...
The attention pattern for dialect $s$ should recover the normalized adjacency submatrix $\tilde{A}^{(s)} = D^{-1} A^{(s)}$. The attention mechanism computes:
...
- Which relation types (dialects) hold between concepts
