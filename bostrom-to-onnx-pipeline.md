...
$$h^* = |\text{Dialect}(G)| \geq 12 \quad \text{(from [[dialect]] registry)}$$
...
For each layer $l \in [1, L^*]$ and each dialect $s \in \text{Dialect}(G)$, we derive attention weight matrices $W_Q^{(l,s)}, W_K^{(l,s)}, W_V^{(l,s)} \in \mathbb{R}^{d^* \times d^*}$.

**Dialect adjacency submatrix:**
...
The optimal $W_Q^{(s)}, W_K^{(s)}$ project particle embeddings into a space where their inner product recovers the dialect-$s$ attention pattern. By the Eckart-Young theorem, the rank-$d^*$ approximation that minimizes reconstruction error is given by the truncated SVD of the dialect-projected embedding product:
...
**Complexity per dialect:** $O(|E_s| \cdot d^*) = O(67{,}633 \times 300) = 2 \times 10^7$ operations.
...
- $|\text{Dialect}| \uparrow$ → $h^* \uparrow$ → finer relation types
