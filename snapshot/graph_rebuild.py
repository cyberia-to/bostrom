...
def parse_tx_events(events, h, ts, txh):
    """Pair particleFrom/particleTo cyberlink events with the neuron event
    that follows them, one tx's worth at a time. A cyberlink msg emits an
    event per (from, to) pair first, then one event naming the neuron that
    signed the whole batch; pending pairs flush against that neuron."""
    rows = []
    pending = []
    for ev in events:
