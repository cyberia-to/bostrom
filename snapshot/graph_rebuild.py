...
def parse_tx_events(events, height, ts, txh):
    """Pair each cyberlink particleFrom/particleTo event with the neuron event
    that follows it in the same tx, into rows [from, to, neuron, height, ts, txh].

    A particleFrom event without a matching particleTo (or vice versa) is
    dropped rather than raising, so one malformed event cannot abort a
    rebuild that is otherwise hours into a 25M-block replay.
    """
    rows = []
    pending = []
    for ev in events:
        if ev["type"] != "cyberlink":
            continue
        ...
        if "particleFrom" in attrs and "particleTo" in attrs:
            ...
            for f, t in pending:
                rows.append([f, t, attrs["neuron"], height, ts, txh])
            pending = []
    for f, t in pending:  # neuron event missing (should not happen)
        rows.append([f, t, "", height, ts, txh])
    return rows
...
def work(h):
    ...
    for i, txr in enumerate(txrs):
        ...
        rows.extend(parse_tx_events(txr.get("events", []), h, ts, txh))
    return ("OK", h, rows)
...
def main():
    ...
    last = 0
    try:
        last = int(open("/tmp/rebuild_progress.txt").read().strip())
    except Exception:
        pass
    ...
    w = csv.writer(out)
    if new:
        w.writerow(["particle_from", "particle_to", "neuron", "height", "timestamp", "transaction_hash"])
